...
(defun eval-expr (expr &optional context (type (expr-type expr context)))
  (labels ((and-op (args) (and (eval-subexpr (car args))
	   ...
	   (call (op args)
	     (case op
	       ...
	       (t (apply (symbol-function op) (mapcar #'eval-subexpr args)))))
	   (eval-subexpr (expr)
	     (cond ((consp expr) (call (car expr) (cdr expr)))
		   ((symbolp arg) (case arg
				    ...
				    (false nil)
				    (nil nil)
				    (t (acase (lookup-binding arg context)
					      (true t)
					      (false nil)
					      (t it)))))
		   (t arg))))
