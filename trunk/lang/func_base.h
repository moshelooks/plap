...
#ifndef PLAP_LANG_FUNC_BASE_H__
#define PLAP_LANG_FUNC_BASE_H__
...
std::vector<func_base*>& ids(); //global repo mapping functions to ids
} //namespace lang_private

struct func_base : boost::noncopyable {
 //fixme - look for gaps in ids() and use them
  func_base() : _id(lang_private::ids().size()) { 
    lang_private::ids().push_back(this); 
  }
  virtual ~func_base() { lang_private::ids()[_id]=NULL; }
...
inline func_base* id2func(id_t s) {
  //  std::cout << s << " "  << func_base::_ids.size() << std::endl;
